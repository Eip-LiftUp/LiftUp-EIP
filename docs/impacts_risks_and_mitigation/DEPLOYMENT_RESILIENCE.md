...
- App continues working offline (core functionality)
- Workouts recorded locally
- Recommendations generated by on-device Rust engine
- Data queued for sync (persistent queue)
- Cannot create new account (requires API)
- Cross-device sync unavailable
- UI Banner: "Working offline - data will sync when connected"
...
- Data Transfer Reduction: Offline-first, compression, incremental sync (-90% bandwidth)
...
- Technical opportunities leveraged (offline-first, Rust efficiency)
